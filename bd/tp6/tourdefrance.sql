...
insert into COUREURS values (361,'BAZIRE','Jean-Michel','COFIDIS','FRA');
...
--SELECT NomCoureur, PrenomCoureur FROM COUREURS;

-- SELECT NomEquipe FROM EQUIPES Natural join COUREURS where NomCoureur = 'ULLRICH';

-- SELECT NomCoureur FROM EQUIPES Natural join COUREURS where NomEquipe = 'COFIDIS';

-- SELECT NomCoureur, PrenomCoureur FROM COUREURS where CodePays = 'FRA';

-- SELECT NomCoureur, to_char(TempsRealise, 'hh:mi:ss') temps_etape3 FROM TEMPS Natural join COUREURS where NomCoureur = 'JALABERT' and NumEtape = 3;

-- SELECT NomCoureur, to_char(TempsRealise, 'hh:mi:ss') temps_etape3  FROM TEMPS Natural join COUREURS Natural join EQUIPES Natural join ETAPES where DirecteurSportif = 'Manolo SAIZ' and VilleDepart = 'Rouen';
